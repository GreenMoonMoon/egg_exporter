...
from .texture_processor import (
    SimpleTextures, TextureBaker, RawTextures, PbrTextures,
)
from .utils import convertFileNameToPanda, eggSafeName
...
class Group:
    ...
    def update_joints_data(self, actor_data_list=None):
        if not actor_data_list:
            actor_data_list = []
            hierarchy_to_list(
                self, actor_data_list, base_filter=EGGActorObjectData,
            )
           ...
           and self.object.__class__ == bpy.types.Bone:
            ...
            for ad in actor_data_list:
                if self.object.name in ad._yabee_object.joint_vtx_ref.keys():
                    vref.append(
                        ad._yabee_object.joint_vtx_ref[self.object.name],
                    )
            self._yabee_object = EGGJointObjectData(
                self.object, vref, self.arm_owner,
            )
        ...
        if self.object:
            for prop in self.object.game.properties:
                ...
                if normalized in ('collide', 'objecttype'):
                    ...
                    'collide-mask', 'from-collide-mask', 'into-collide-mask',
                    'bin', 'draw-order', 'occluder', "portal",
                ):
                    vals = ('  ' * level, prop.name, prop.value)
    ...
    def get_full_egg_str_arr(self, level=0):
        ...
        if self.object:
            if self.object.__class__ == bpy.types.Bone:
                ...
                if self.object.type == 'MESH' and (self.object.data.shape_keys and len(self.object.data.shape_keys.key_blocks) > 1):
            ...
            if spline.type == 'NURBS':
                ...
                cur_str = '<NURBSCurve> {{\n  <Scalar> subdiv {{ {} }}\n  <Order>\
                     ...
                     <Ref> {{ {} }} \n  }}\n}}\n'.format(
                        ...
                        spline.order_u, ' '.join(map(str2f, knots)),
...
# def write_out(fname, preferences):
def write_out(fname, **kwargs):
    ...
    # imp.reload(sys.modules[lib_name + '.texture_processor'])  # ??? why reload
    # imp.reload(sys.modules[lib_name + '.utils'])  # ??? why reload
    ...
    old_data = {}
    for d in (
        bpy.data.materials, bpy.data.objects, bpy.data.textures,
